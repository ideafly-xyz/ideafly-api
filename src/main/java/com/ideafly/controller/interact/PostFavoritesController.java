...
public class PostFavoritesController {
    ...
    public R<?> getFavoriteJobs(@RequestBody JobListInputDto request, HttpServletRequest httpRequest) {
    ...
    public R<Boolean> toggleFavorite(@RequestBody @Valid JobFavoriteInputDto request, HttpServletRequest httpRequest) {
        String userId = RequestUtils.getCurrentUserId(httpRequest);
        if (userId == null) {
            return R.error("用户未登录");
        }
